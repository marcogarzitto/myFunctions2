...
my_paired_anova <- function (y, time, observations, void_string = '-', alpha_value = 0.050, multiple_alphas = c(0.050, 0.010, 0.001), wise = TRUE, direction = 'Stable')
{
 ...
 if (p_value < alpha_value)
 {
  ...
  ES <- effectsize::omega_squared(MODEL, partial = FALSE, alternative = test_direction, ci = 0.950, generalized = FALSE)
  effect_size <- paste(my_nice(ES$Omega2, decimals = 3, text = "\u03C9\u00B2", with_equal_sign = TRUE, with_sign = FALSE, min_value = -1000, max_value = 1000, void_string = void_string),
