#' my_paired_pearson_r
#'
#' Function to do a withween-subjects Pearson's correlation.
#'
#' @param y Numeric vector. Dependent variable (using long format for data-frame). Default: None.
#' @param time Numeric vector. Independent variable (using long format for data-frame). Default: None.
...
#' @export
my_paired_pearson_r <- function (y, time, void_string = '-', alpha_value = 0.050, multiple_alphas = c(0.050, 0.010, 0.001), wise = TRUE, direction = 'Stable')
{
 result <- void_string
 p_value <- 1.0
