...
#' @export
my_paired_univariate <- function (dv, iv, void_string = '-', alpha_value = 0.050, multiple_alphas = c(0.050, 0.010, 0.001), wise = TRUE, direction = 'Stable')
{
 result <- void_string
 p_value <- 1.0
     ...
     (dv_classification == 'Dummy'))
 {
  ...
  if (iv_classification == 'Dichotomous') { RESULTS <- my_paired_ttest(y = dv, time = iv, void_string = void_string, alpha_value = alpha_value, multiple_alphas = multiple_alphas, wise = wise, direction = direction) }
  if (iv_classification == 'Polytomous') { RESULTS <- my_paired_anova(y = dv, time = iv, void_string = void_string, alpha_value = alpha_value, multiple_alphas = multiple_alphas, wise = wise, direction = direction) }
 }
 #
 if ((dv_classification == 'Dichotomous'))
 {
  if (iv_classification == 'Continuous') { RESULTS <- my_paired_ttest(y = iv, time = dv, void_string = void_string, alpha_value = alpha_value, multiple_alphas = multiple_alphas, wise = wise, direction = direction) }
  if (iv_classification == 'Poor continuous') { RESULTS <- my_paired_ttest(y = iv, time = dv, void_string = void_string, alpha_value = alpha_value, multiple_alphas = multiple_alphas, wise = wise, direction = direction) }
  if (iv_classification == 'Dummy') { RESULTS <- my_paired_ttest(y = iv, time = dv, void_string = void_string, alpha_value = alpha_value, multiple_alphas = multiple_alphas, wise = wise, direction = direction) }
  if (iv_classification == 'Dichotomous') { RESULTS <- my_paired_fisher(a = dv, b = iv, void_string = void_string, alpha_value = alpha_value, multiple_alphas = multiple_alphas, direction = direction) }
